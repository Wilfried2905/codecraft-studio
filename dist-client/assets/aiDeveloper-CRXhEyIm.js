...
`}),i}getAgent(e){const t=this.agents.find(s=>s.id===e);if(!t)throw new Error(`Agent '${e}' not found`);return t}getAllAgents(){return this.agents}}class f extends Error{constructor(e,t,s=500,i){super(t),this.code=e,this.message=t,this.statusCode=s,this.details=i,this.name="AppError"}}class m extends f{constructor(e,t){super("VALIDATION_ERROR",e,400,t),this.name="ValidationError"}}function T(r){return r instanceof f?`❌ **${r.name}**
...
Une erreur inattendue est survenue.`}function S(r,e){const t=new Date().toISOString(),s=`[${e}]`;console.error(`${t} ${s} Error:`,r),r instanceof f&&(console.error(`  Code: ${r.code}`),console.error(`  Status: ${r.statusCode}`),r.details&&console.error("  Details:",r.details))}function x(r){const e=[],t=[];if(!r||r.trim().length===0)return e.push("Le code généré est vide"),{isValid:!1,errors:e,warnings:t};r.includes("<html")||t.push("Pas de balise <html> détectée"),r.includes("<head")||t.push("Pas de balise <head> détectée"),r.includes("<body")||t.push("Pas de balise <body> détectée");const s=r.match(/<(?!\/)[\w-]+(?:\s[^>]*)?>/g)||[],i=r.match(/<\/[\w-]+>/g)||[],n=s.map(d=>{var l;return(l=d.match(/^<([\w-]+)/))==null?void 0:l[1]}).filter(Boolean),a=i.map(d=>{var l;return(l=d.match(/^<\/([\w-]+)>/))==null?void 0:l[1]}).filter(Boolean),o=["br","hr","img","input","meta","link"],c=n.filter(d=>!o.includes(d||"")&&n.filter(l=>l===d).length>a.filter(l=>l===d).length);c.length>0&&t.push(`Balises potentiellement non fermées: ${c.join(", ")}`),(r.includes("<script>alert")||r.includes("javascript:"))&&t.push("Code JavaScript potentiellement dangereux détecté");const u=(r.match(/style\s*=\s*"/g)||[]).length;return u>10&&t.push(`Beaucoup de styles inline (${u}). Considérez utiliser des classes CSS.`),{isValid:e.length===0,errors:e,warnings:t,sanitized:r}}function C(r){let e=r;return["onerror","onload","onclick","onmouseover"].forEach(s=>{const i=new RegExp(`${s}\\s*=\\s*["'][^"']*["']`,"gi");e=e.replace(i,"")}),e=e.replace(/javascript:/gi,""),e=e.replace(/data:text\/html/gi,""),e}function w(r,e,t){const s=x(r),i={isValid:!0,errors:[],warnings:[]},n={isValid:!0,errors:[],warnings:[]};return{isValid:s.isValid&&i.isValid&&n.isValid,errors:[...s.errors.map(a=>`HTML: ${a}`),...i.errors.map(a=>`CSS: ${a}`),...n.errors.map(a=>`JS: ${a}`)],warnings:[...s.warnings.map(a=>`HTML: ${a}`),...i.warnings.map(a=>`CSS: ${a}`),...n.warnings.map(a=>`JS: ${a}`)],sanitized:C(r)}}function E(r){if(r.isValid&&r.warnings.length===0)return"✅ **Code valide** - Aucun problème détecté";let e="";return r.isValid||(e+=`❌ **Erreurs détectées**
...
`})),e}class R{constructor(){g(this,"intentAnalyzer");g(this,"clarificationEngine");g(this,"agentOrchestrator");g(this,"conversationState");this.intentAnalyzer=new v,this.clarificationEngine=new b,this.agentOrchestrator=new A,this.conversationState={awaitingClarification:!1}}async process(e,t){try{if(!e||e.trim().length===0)throw new m("Le prompt ne peut pas être vide");if(e.length>1e4)throw new m("Le prompt est trop long (maximum 10000 caractères)");const{intent:s,requirements:i}=await this.intentAnalyzer.analyze(e,t);return this.conversationState.awaitingClarification?await this.handleClarificationResponse(e):s.type==="question"?{type:"clarification",message:this.generateQuestionResponse(e)}:s.needsClarification?this.requestClarification(s,i):await this.executeGeneration(i)}catch(s){return S(s,"AIDeveloper.process"),{type:"error",message:T(s)}}}requestClarification(e,t){const s=this.clarificationEngine.generateQuestions(e,t);return this.conversationState={awaitingClarification:!0,currentIntent:e,currentRequirements:t},{type:"clarification",message:s.questions.join(`

`),requirements:t}}async handleClarificationResponse(e){if(!this.conversationState.currentRequirements)return{type:"error",message:"Erreur: État de clarification invalide"};const t=this.clarificationEngine.parseUserResponse(e,this.conversationState.currentRequirements);return this.conversationState.awaitingClarification=!1,await this.executeGeneration(t)}async executeGeneration(e){const t=this.agentOrchestrator.createPlan(e),s=this.generatePlanMessage(t,e),i=await this.agentOrchestrator.execute(t,e),n=this.agentOrchestrator.mergeResults(i,e),a=w(n),o=E(a),c=this.generateExecutionMessage(i,e);return{type:"execution",message:s+`
...
Que voulez-vous créer aujourd'hui ? 🚀`:`Je suis prêt à créer votre application ! Décrivez-moi ce que vous voulez, ou demandez de l'aide en tapant "aide" ou "comment ça marche".`}resetState(){this.conversationState={awaitingClarification:!1}}}export{R as AIDeveloper};
//# sourceMappingURL=aiDeveloper-CRXhEyIm.js.map
